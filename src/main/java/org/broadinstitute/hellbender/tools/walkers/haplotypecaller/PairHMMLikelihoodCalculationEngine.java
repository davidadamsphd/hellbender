...
public final class PairHMMLikelihoodCalculationEngine implements ReadLikelihoodCalculationEngine {
    ...
    private void initializePairHMM(final List<Haplotype> haplotypes, final Map<String, List<GATKRead>> perSampleReadList) {
        int readMaxLength = 0;
        for( final Map.Entry<String, List<GATKRead>> sample : perSampleReadList.entrySet() ) {
            for( final GATKRead read : sample.getValue() ) {
                final int readLength = read.getLength();
                if( readLength > readMaxLength ) {
                    readMaxLength = readLength;
                }
            }
        }
        int haplotypeMaxLength = 0;
        for( final Haplotype h : haplotypes ) {
            if( h.getBases().length > haplotypeMaxLength ) {
                haplotypeMaxLength = h.getBases().length;
            }
        }
    ...
    private void computeReadLikelihoods( final LikelihoodMatrix<Haplotype> likelihoods) {
        ...
        pairHMM.computeLikelihoods(likelihoods, processedReads, gapContinuationPenalties);
    ...
    private Map<GATKRead, byte[]> buildGapContinuationPenalties(final List<GATKRead> reads, final byte gapPenalty) {
    ...
    @Deprecated
    public static <A extends Allele> double[][] computeDiploidHaplotypeLikelihoods( final String sample,
                                                                 final ReadLikelihoods<A> readLikelihoods,
                                                                 final List<A> alleleOrdering,
                                                                 final boolean normalize ) {
        return computeDiploidHaplotypeLikelihoods(Collections.singleton(sample), readLikelihoods, alleleOrdering, normalize);
    }

    static <A extends Allele> double[][] computeDiploidHaplotypeLikelihoods( final Set<String> samples,
                                                                 final ReadLikelihoods<A> readLikelihoods,
                                                                 final List<A> alleleOrdering,
                                                                 final boolean normalize) {
        ...
        final ListIterator<A> alleleIterator = alleleOrdering.listIterator();
        int nextAlleleIndex = 0;
        while (alleleIterator.hasNext()) {
            if ((alleleIndices[nextAlleleIndex++] = readLikelihoods.indexOfAllele(alleleIterator.next())) == -1) {
        ...
        for(final String sample : samples) {
            final int sampleIndex = readLikelihoods.indexOfSample(sample);
            if (sampleIndex == -1) {
                throw new IllegalArgumentException("the sample provided is not in the likelihood collection");
            }
            final LikelihoodMatrix<? extends Allele> sampleLikelihoods = readLikelihoods.sampleMatrix(sampleIndex);
            final int sampleReadCount = readLikelihoods.sampleReadCount(sampleIndex);
            for( int i = 0; i < numHaplotypes; i++ ) {
                final int i_allele = alleleIndices[i];
                for( int j = 0; j <= i; j++ ) {
                    final int j_allele = alleleIndices[j];
                    double haplotypeLikelihood = 0.0;
                    for (int k = 0; k < sampleReadCount; k++) {
                        haplotypeLikelihood += MathUtils.approximateLog10SumLog10(sampleLikelihoods.get(i_allele, k),
                                sampleLikelihoods.get(j_allele, k)) + MathUtils.LOG_ONE_HALF;
                    }
                    haplotypeLikelihoodMatrix[i][j] += haplotypeLikelihood;
                }
            }
        }
    ...
    private void initializePCRErrorModel() {
        if ( pcrErrorModel == PCRErrorModel.NONE || !pcrErrorModel.hasRateFactor() ) {
            return;
        }
