...
public class SelectVariants extends VariantWalker {
    ...
    @Argument(fullName = StandardArgumentDefinitions.LENIENT_LONG_NAME,
                        ...
                        doc = "Leneint processing of VCF files", common = true, optional = true)
     ...
     *
     * Was public void initialize()
     */
    @Override
    public void onTraversalStart() {
        FeatureInput<? extends Feature> featureInput = getDrivingVariantsFeatureInput();

        Map<String, VCFHeader> vcfRods = new HashMap<String, VCFHeader>();
        ...
        Set<VCFHeaderLine> headerLines = createVCFHeaderLineList(vcfRods);
        ...
        discordanceOnly = discordanceTrack == null ? false : true;
        if (discordanceOnly) {
            logger.info("Selecting only variants discordant with the track: " + discordanceTrack.getName());
        }

        concordanceOnly = concordanceTrack == null ? false : true;
        if (concordanceOnly) {
        ...
        // Prepare the interval list for the interval filter
    ...
    public void apply(VariantContext vc, ReadsContext readsContext, ReferenceContext ref, FeatureContext featureContext) {
        ...
        // TODO: many of these tests could also be factored out into re-useable filter predicates with immutable state
    ...
    /**
     * Create filters for sample name, variant type, id, and genomic interval.
     */
    protected VariantFilter makeVariantFilter() {
        ...
        if (samples.size() != 0) {
            tmpFilter = new SampleNamesVariantFilter(samples);
    ...
    /**
     * Determine the sample names to be included(/excluded) in the output.
     */
    private TreeSet<String> createSampleNameInclusionList(Map<String, VCFHeader> vcfHeaders) {
        TreeSet<String> vcfSamples = new TreeSet<>(getSampleList(vcfHeaders, GATKVariantContextUtils.GenotypeMergeType.REQUIRE_UNIQUE));

        Collection<String> samplesFromFile = getSamplesFromFiles(sampleFiles);
        Collection<String> samplesFromExpressions = matchSamplesExpressions(vcfSamples, sampleExpressions);
    ...
    private Set<VCFHeaderLine> createVCFHeaderLineList(Map<String, VCFHeader> vcfHeaders) {
        ...
        Set<VCFHeaderLine> headerLines = VCFUtils.smartMergeHeaders(vcfHeaders.values(), true);
    ...
    /*
     * TODO: the GATK code does this - seems pretty sketchy...
     */
    ...
    // @TODO CMN - this  was pulled from GATK GenomeAnalysisEngine
    /**
    ...
    private SampleDB initializeSampleDB() {
        // TODO CMN - add a pedigreeValidationType arg
    ...
    // TODO: lifted from Adam's VariantFiltration branch - lift to VariantWalker ?
    /**
    ...
    // TODO : next 3 methods (getSample*) lifted from GATK::SampleUtils - factor/port ?
