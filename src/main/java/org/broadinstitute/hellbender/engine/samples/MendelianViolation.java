...
import org.broadinstitute.hellbender.engine.samples.Sample;
    ...
    private void updateViolations(String familyId, String motherId, String fatherId, String childId, VariantContext vc){
        ...
        if (gMom == null || gDad == null || gChild == null){
            if(abortOnSampleNotFound)
                throw new IllegalArgumentException(String.format("Variant %s:%d: Missing genotypes for family %s: mom=%s dad=%s family=%s",
                        vc.getContig(), vc.getStart(), familyId, motherId, fatherId, childId));
            else
                return;
        }
        //Count No calls
        ...
        else if (minGenotypeQuality>0 && (gMom.getGQ()   < minGenotypeQuality ||
